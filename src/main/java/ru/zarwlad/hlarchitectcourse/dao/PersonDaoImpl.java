...
public class PersonDaoImpl implements PersonDao {
    ...
    private static final String SQL_FIND_BY_ID = "SELECT * FROM persons JOIN cities ON persons.city_id = cities.id WHERE persons.id = ?";
    private static final String SQL_FIND_ALL_PAGED = "SELECT * FROM persons JOIN cities ON persons.city_id = cities.id LIMIT ? OFFSET ?";
    private static final String SQL_CREATE = "INSERT INTO persons (email, login, password, name, surname, age, gender, city_id) " +
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    private static final String SQL_UPDATE = "UPDATE persons SET email = ?, login = ?, name = ?, " +
            "surname = ?, age = ?, gender = ?, city_id = ? WHERE id = ?";
    private static final String SQL_DELETE = "DELETE FROM persons WHERE id = ?";
    private static final String SQL_FIND_PERSON_INTERESTS = "SELECT person_id, interest_id as id, tag FROM persons_interests pi " +
            "JOIN interests i ON pi.interest_id = i.id WHERE person_id = ?";
    ...
    private static final String SQL_FIND_BY_LOGIN = "SELECT * FROM persons JOIN cities ON persons.city_id = cities.id WHERE persons.login = ?";

    private static final String SQL_FIND_ALL_INTERESTS_OF_PERSONS = "SELECT * FROM persons_interests pi " +
            "JOIN interests i ON pi.interest_id = i.id WHERE person_id IN (:personIds)";
    ...
    public Optional<List<PersonInterest>> findAllInterestsOfPersonsByPersonIds(List<Long> personIds) {
        ...
        return Optional.of(namedParameterJdbcTemplate.query(
                SQL_FIND_ALL_INTERESTS_OF_PERSONS,
                parameter,
                new PersonInterestMapper())
        );
    }
}
